...
- 🦀 **Optional Rust core (Robyn-style)** — routing, JSON encoding, and the dev server can run on a compiled PyO3/xitca-web extension using a multi-process worker model, with automatic fallback to pure Python. Works on any standard Python 3.8+, no special interpreter build required
...
extension (`jet_core`, built with **PyO3 0.29.0** + **xitca-web /
Tokio**) instead of pure Python. It uses the same GIL-workaround
strategy as [Robyn](https://github.com/sansyrox/robyn): one OS process
per CPU core (each with its own interpreter and GIL, bound to the same
port via `SO_REUSEPORT`), plus a fully async, GIL-free I/O layer
inside each process that only touches Python for the moment your
handler actually runs. **No special Python build required** — it
targets any standard CPython 3.8+ via `abi3`. Without it, Jet falls
back to a single-process pure-Python server automatically, and the
startup banner tells you which engine and how many workers are active.
